<div class="max-w-2xl mx-auto">
    <x-card>
        <x-slot:header>
         ...
         <form id="armada-update-{{ $armada->id }}" wire:submit="save" class="space-y-4">
            <div>
                <x-input label="Nama *" x-ref="name" wire:model="armada.name" required />
            </div>

            <div>
                <x-input label="Slug *" wire:model="armada.slug" required />
            </div>

            <div>
                <x-input label="Deskripsi *" wire:model="armada.description" required />
            </div>
        ...
        <x-slot:footer>
            <x-button type="submit" form="armada-update-{{ $armada->id }}">
                Save
            </x-button>
        </x-slot:footer>
    </x-card>
