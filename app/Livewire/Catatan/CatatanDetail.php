...
class CatatanDetail extends Component
{
    ...
    public function mount($catatanid)
    {
        $this->catatan = Catatan::find($catatanid);
        $catatanEdit = Catatan::find($catatanid);
        $this->status = $catatanEdit->status;
        $this->type = $catatanEdit->type;
        $this->categories = $catatanEdit->categories;
        $this->tags = $catatanEdit->tags;
        $this->tanggal = $catatanEdit->tanggal;
        $this->target = $catatanEdit->target;
        $this->collected = $catatanEdit->collected;
    }

    public function render()
    {
        return view('livewire.catatan.catatan-detail');
    }
    ...
    public function rules(): array
    {
        return [
            ...
            'catatan.slug' => [
                ...
                'string',
                'max:255',
                Rule::unique('catatans', 'slug')->ignore($this->catatan->id),
            ],
            'catatan.catatan' => [
                'required',
                'string',
